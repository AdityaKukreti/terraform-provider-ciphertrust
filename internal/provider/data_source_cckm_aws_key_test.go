...
func TestCckmAWSDataSourceKey(t *testing.T) {
	...
	datasourceConfig := `
		...
		data "ciphertrust_aws_keys_list" "by_aws_key_id" {
			filters = { "keyid" = ciphertrust_aws_key.aws_key.aws_param.aws_key_id }
		}
		...
		data "ciphertrust_aws_keys_list" "by_key_id_and_region" {
			filters = {
				"keyid"  = ciphertrust_aws_key.aws_key.aws_param.aws_key_id
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(dsByAwsKeyID, "keys.0.customer_master_key_spec", "SYMMETRIC_DEFAULT"),
					resource.TestCheckResourceAttr(dsByAwsKeyID, "keys.0.description", "Updated"),
					resource.TestCheckResourceAttr(dsByAwsKeyID, "keys.0.enabled", "true"),
					resource.TestCheckResourceAttr(dsByAwsKeyID, "keys.0.key_state", "Enabled"),
					resource.TestCheckResourceAttr(dsByAwsKeyID, "keys.0.key_usage", "ENCRYPT_DECRYPT"),
					resource.TestCheckResourceAttr(dsByAwsKeyID, "keys.0.origin", "AWS_KMS"),
					resource.TestCheckResourceAttr(dsByAwsKeyID, "keys.0.auto_rotate", "false"),
					resource.TestCheckResourceAttr(dsByAwsKeyID, "keys.0.tags.%", "0"),
					resource.TestCheckResourceAttr(dsByAwsKeyID, "keys.0.alias.#", "2"),
					resource.TestCheckResourceAttrSet(dsByAwsKeyID, "keys.0.arn"),
					// aws_param block
					...
					resource.TestCheckResourceAttr(dsByAwsKeyIDAndRegion, "keys.0.customer_master_key_spec", "SYMMETRIC_DEFAULT"),
					resource.TestCheckResourceAttr(dsByAwsKeyIDAndRegion, "keys.0.key_state", "Enabled"),
				),
			},
		},
	})
