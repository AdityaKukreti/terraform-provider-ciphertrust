...
`auto_rotation_period_in_days`, and `tags`), `key_policy`, `enable_rotation`, `enable_key`,
`auto_rotate`, and `primary_region` to their current values so that Terraform detects
no diff.
...
if the key does not use AWS-managed rotation, or omit `aws_param.tags` if the key has no tags.
